...
export default function AssetsPage() {
  ...
  async function archive(id: string, name: string) {
    if (!confirm(`Archive "${name}"? Its full history is kept — you can restore it from Archived.`)) return;
                      ...
                      {isStaff && (
                        <>
                          ...
                          <button onClick={() => archive(a.id, a.name)} title="Archive (keeps history)"
                            className="p-1 rounded-lg hover:bg-amber-500/10 hover:text-amber-500" style={{ color: "var(--text-secondary)" }}>
                            <Archive size={14} />
                          </button>
                        </>
                      )}
